 ...
 */
console.log('SMO Chat: smo-chat-interface.js loaded');
...
class SMOChatInterface {
    ...
    createFallbackContainer() {
        ...
        container.classList.add('smo-chat-fallback');
        
        // Check if we're in an embedded context (like create post tab)
        const isEmbeddedContext = this.isInEmbeddedContext();
        if (isEmbeddedContext) {
            container.dataset.context = 'embedded';
            ...
            // Default floating position for non-embedded contexts
    ...
    /**
     * Check if the chat is being embedded in a larger interface
     */
    isInEmbeddedContext() {
        // Check if we're in a tab, modal, or admin interface
        const currentPath = window.location.pathname;
        const isCreatePost = currentPath.includes('create-post') ||
                           currentPath.includes('enhanced-create-post') ||
                           document.querySelector('#smo-chat-container') !== null;
        
        // Check for admin interface context
        const isAdminContext = document.body.classList.contains('wp-admin') ||
                              document.querySelector('.wrap') !== null ||
                              document.querySelector('#wpadminbar') !== null;
        
        // Check for specific container presence that suggests embedded context
        const hasMainContainer = document.querySelector('.smo-content-body') !== null ||
                               document.querySelector('.smo-create-post-layout') !== null;

        return isCreatePost || (isAdminContext && hasMainContainer);
    }

    async init() {
        console.log('SMO Chat: init called');
        try {
            ...
            // Set context attribute based on detection
            const isEmbedded = this.isInEmbeddedContext();
            if (isEmbedded) {
                this.container.setAttribute('data-context', 'embedded');
            }

            // Ensure container has proper classes for styling
            if (!this.container.classList.contains('smo-chat-container') &&
                !this.container.classList.contains('smo-chat-embedded')) {
                this.container.classList.add('smo-chat-container');
            }
    ...
    render() {
        // Check if container already has smo-chat-container class
        const hasChatContainerClass = this.container.classList.contains('smo-chat-container') || this.container.classList.contains('smo-chat-embedded');
        
        if (hasChatContainerClass) {
            // If container already has the right classes, render content directly
            ...
            // If container doesn't have the right classes, wrap content in smo-chat-container
            this.container.innerHTML = `
        ...
        // Ensure the container has the proper role and aria attributes
    ...
    setupRealtimeUpdates() {
        console.log('SMO Chat: Setting up real-time updates using polling system...');
        ...
        // Use polling system instead of WebSocket
        this.setupPollingSystem();
    }

    async setupPollingSystem() {
        console.log('SMO Chat: Initializing polling system...');
        console.log('SMO Chat: window.SMOPolling defined:', typeof window.SMOPolling !== 'undefined');

        // Check if polling client is available
        if (typeof window.SMOPolling !== 'undefined') {
            try {
                // Connect to polling system
                await window.SMOPolling.connect();
                
                // Subscribe to chat-related channels
                await window.SMOPolling.subscribe('chat_updates');
                await window.SMOPolling.subscribe(`user_${this.options.userId}_chat`);
                
                // Set up message handlers
                window.SMOPolling.onMessage((message) => {
                    this.handleRealtimeUpdate(message);
                });
                
                console.log('SMO Chat: Successfully connected to polling system');
                
            } catch (error) {
                console.error('SMO Chat: Failed to connect to polling system:', error);
                this.fallbackToPolling();
            }
        } else {
            console.warn('SMO Chat: Polling client not available, falling back to manual polling');
            this.fallbackToPolling();
        }
    }
    
    // Keep old WebSocket setup for backwards compatibility logging
    async setupWebSocket() {
        console.log('SMO Chat: WebSocket setup called - this should not happen in the new system!');
        if (typeof WebSocket !== 'undefined') {
            try {
                const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
                const wsUrl = this.options.wsUrl || `${protocol}//${window.location.hostname}:8080/chat`;
                console.log(`SMO Chat: Attempting WebSocket connection to ${wsUrl} (this will fail - system uses polling)`);
                this.ws = new WebSocket(wsUrl);

                this.ws.onopen = () => {
                    console.log('SMO Chat: WebSocket connected successfully (unexpected!)');
                };
                ...
                this.ws.onclose = (event) => {
                    console.log('SMO Chat: WebSocket connection closed, using polling fallback');
                    this.fallbackToPolling();
                };

                this.ws.onerror = (error) => {
                    console.log('SMO Chat: WebSocket error (expected - using polling instead):', error);
                    this.fallbackToPolling();
                };
                ...
                setTimeout(() => {
                    if (this.ws && this.ws.readyState === WebSocket.CONNECTING) {
                        this.ws.close();
                        console.log('SMO Chat: WebSocket connection timeout, using polling');
                        this.fallbackToPolling();
                    }
                }, 5000);

            } catch (error) {
                console.log('SMO Chat: WebSocket setup failed (expected - using polling):', error);
                this.fallbackToPolling();
            }
        } else {
            console.log('SMO Chat: WebSocket not supported, using polling');
            this.fallbackToPolling();
        }
    }
