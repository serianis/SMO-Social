...
class Manager {
    ...
    /**
     * Get available platforms for use (returns array of Platform objects)
     * This method is called by MagicWizard to populate platform selection
     *
     * @return array Array of Platform objects with get_slug() and get_name() methods
     */
    public function get_available_platforms(): array {
        $result = array();
        
        // Load enabled platforms on demand
        foreach ($this->enabled_platforms as $slug) {
            $platform = $this->get_platform($slug);
            if ($platform) {
                $result[] = $platform;
