 ...
 * Plugin URI: https://github.com/serianis/SMO-Social
...
if (!defined('SMO_SOCIAL_PLUGIN_URL')) {
    if (smo_social_is_wordpress() && function_exists('plugin_dir_url')) {
        ...
        // Standalone mode - auto-detect plugin URL
...
function smo_social_init() {
    ...
    // Initialize Real-Time system (replaces WebSocket with REST API polling)
    if (smo_social_is_wordpress()) {
        if (file_exists(SMO_SOCIAL_PLUGIN_DIR . 'includes/RealTime/RealTimeManager.php')) {
            try {
                require_once SMO_SOCIAL_PLUGIN_DIR . 'includes/RealTime/RealTimeManager.php';
                if (class_exists('SMO_Social\\RealTime\\RealTimeManager')) {
                    $real_time_manager = new \SMO_Social\RealTime\RealTimeManager();
                    $real_time_manager->initialize();
                    
                    // Store instance globally for access
                    $GLOBALS['smo_real_time_manager'] = $real_time_manager;
                    
                    error_log("SMO Social: Real-Time system initialized successfully");
                }
            } catch (Exception $e) {
                error_log("SMO Social: Error initializing Real-Time system: " . $e->getMessage());
            }
        }
    }
...
function smo_social_activate() {
    ...
    // Initialize Real-Time system on activation
    if (smo_social_is_wordpress()) {
        if (file_exists(SMO_SOCIAL_PLUGIN_DIR . 'includes/RealTime/RealTimeManager.php')) {
            try {
                require_once SMO_SOCIAL_PLUGIN_DIR . 'includes/RealTime/RealTimeManager.php';
                ...
                error_log("SMO Social: Error initializing Real-Time system on activation: " . $e->getMessage());
            }
        }
    }
...
function smo_social_deactivate() {
    ...
    // Cleanup Real-Time system on deactivation
    if (smo_social_is_wordpress() && isset($GLOBALS['smo_real_time_manager'])) {
        try {
            $GLOBALS['smo_real_time_manager']->cleanup_on_deactivation();
        } catch (Exception $e) {
            error_log("SMO Social: Error cleaning up Real-Time system on deactivation: " . $e->getMessage());
        }
    }
...
function smo_social_uninstall() {
    ...
    // Cleanup Real-Time system data on uninstall
    if (smo_social_is_wordpress()) {
        if (file_exists(SMO_SOCIAL_PLUGIN_DIR . 'includes/RealTime/RealTimeManager.php')) {
            try {
                require_once SMO_SOCIAL_PLUGIN_DIR . 'includes/RealTime/RealTimeManager.php';
                if (class_exists('SMO_Social\\RealTime\\RealTimeManager')) {
                    ...
                    $real_time_manager->cleanup_on_deactivation();
                }
            } catch (Exception $e) {
                error_log("SMO Social: Error cleaning up Real-Time system on uninstall: " . $e->getMessage());
            }
        }
    }
