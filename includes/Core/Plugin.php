...
class Plugin {
    ...
    public function ajax_get_websocket_config() {
        ...
        // Attempt 2: Try standalone XAMPP WebSocket server
        if (!$config['url']) {
            try {
                // Check if XAMPP WebSocket server is running on port 8080
                $sock = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
                if ($sock) {
                    socket_set_option($sock, SOL_SOCKET, SO_RCVTIMEO, ['sec' => 1, 'usec' => 0]);
                    $connected = @socket_connect($sock, '127.0.0.1', 8080);
                    ...
                    if ($connected) {
                        ...
                        if (strpos($response, '101') !== false || strpos($response, 'Switching Protocols') !== false) {
                            $config['url'] = 'ws://127.0.0.1:8080';
                            $attempts[] = "XAMPP server: ws://127.0.0.1:8080";
                            error_log('SMO WebSocket: XAMPP WebSocket server detected and working');
                        }
                    }
                ...
                error_log('SMO WebSocket: XAMPP server check failed: ' . $e->getMessage());
                $attempts[] = "XAMPP server check failed: " . $e->getMessage();
            }
        }
